...
impl KeypunkService {
    ...
    pub async fn get_public_key(&self) -> Result<[u8; 32], String> {
        let request = keypunkd::messages::KeypunkdRequest::GetPublicKey;
        let payload =
        ...
        let response: keypunkd::messages::KeypunkdResponse =
            postcard::from_bytes(&response_bytes).map_err(|e| format!("deserialize error: {e}"))?;
        match response {
            keypunkd::messages::KeypunkdResponse::PublicKey { key } => Ok(key),
